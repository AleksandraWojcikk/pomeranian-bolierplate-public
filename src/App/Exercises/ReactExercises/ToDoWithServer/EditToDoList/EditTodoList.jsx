...
export const EditTodoList = ({ backFunction, refreshFunction }) => {
  const [author, setAuthor] = useState('');
  const [title, setTitle] = useState('');
  const [note, setNote] = useState('');
  const [getError, setGetError] = useState(null);

  async function editTodo(paramId) {
    const data = {
      title: title,
      author: author,
      note: note,
    };
    await sendData(`/api/todo/${paramId}`, data, 'PUT')
      .then((response) => {
        backFunction(false);
  ...
  return (
    <>
      <div className="header">
        <p className="titleOfThePage">Edycja zadania.</p>
      </div>
